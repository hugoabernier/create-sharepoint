...
function renderPath(relPath, tokens) {
    let out = relPath;
...
function renderContent(text, tokens) {
...
async function renderTemplateDir(templateDir, targetDir, tokens) {
    ...
    for (const rel of entries) {
        ...
        const isBinary = /\.(png|jpe?g|gif|bmp|ico|webp|woff2?|ttf|eot|pdf)$/i.test(rel);
...
function printSplash() {
    if (!process.stdout.isTTY)
        return; // skip in non-interactive CI
