...
function color(hex: string, s: any) {
  const [r, g, b] = hexToRgb(hex);
  return `\x1b[38;2;${r};${g};${b}m${s}\x1b[0m`;
}
function colorizeChar(ch: string) {
  switch (ch) {
...
export function showSplash(appName: string, subtitle: string) {
...
`);
  ...
  for (const line of art.replace(/\r\n/g, '\n').split('\n')) {
    ...
    let out = '';
    for (const ch of line) out += colorizeChar(ch);
    console.log(out);
  }
