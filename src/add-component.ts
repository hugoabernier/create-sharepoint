...
export async function addComponent(args: { targetDir: string; template: string; variant: string; name: string; }) {
    const tokens = {
    ...
    await copyDirWithTransforms(
        resolveComponentTemplate(args.template, args.variant),
        args.targetDir,
        tokens,
        { mode: "add", shouldCopy: shouldCopyOnAdd }
    );
